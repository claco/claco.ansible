...
clean: clean-ansible clean-bazel clean-docker clean-python clean-venv
...
.PHONY: clean-bazel
clean-bazel: ## clean bazel related files
clean-bazel:
	@bazel clean --async
...
.PHONY: lint
lint: ## lint project
lint: lint-yaml lint-ansible lint-python lint-security lint-git
...
lint-ansible-playbooks:
	@docker run -ti -v $(PWD):/workspace ansible-lint roles/*/molecule/*/*.yml playbooks/*.yml playbooks/*/*.yml
...
lint-ansible-roles:
	@docker run -ti -v $(PWD):/workspace ansible-lint roles/*/
...
lint-depends:
	@bazel build //images:linting
	@bazel run //images:linting
...
lint-git:
	@docker run -v $(PWD):/workspace gitlint
	@.venv/bin/gitlint
...
lint-yaml:
	@docker run -ti -v $(PWD):/workspace yamllint
	#@.venv/bin/yamllint -c .yamllint --strict .ansible-lint .yamllint .

.PHONY: galago
galago: ## configure galago pro host
galago: name=hosts/galago
galago: playbook

.PHONY: pinebook
pinebook: ## configure pinebook pro host
pinebook: name=hosts/pinebook
pinebook: playbook

.PHONY: playbook
playbook: ## run an existing playbook
playbook:
	@.venv/bin/ansible-playbook playbooks/$(name).yml

.PHONY: role
role: ## create a new role in this project - make roles name=foop
role:
	@.venv/bin/ansible-galaxy init ./roles/claco.$(name)
